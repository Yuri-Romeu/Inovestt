...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $senha = !empty($_POST['senha']) ? $mysqli->real_escape_string($_POST['senha']) : $user_data['senha'];
    $sexo = !empty($_POST['sexo']) ? $mysqli->real_escape_string($_POST['sexo']) : $user_data['sexo'];
    $data_nascimento = !empty($_POST['data']) ? $mysqli->real_escape_string($_POST['data']) : (isset($user_data['data_de_nascimento']) ? $user_data['data_de_nascimento'] : '');
    ...
    $cpf_cnpj = !empty($_POST['cpf_cnpj']) ? $mysqli->real_escape_string($_POST['cpf_cnpj']) : $user_data['cpf_cnpj'];
    $telefone = !empty($_POST['telefone']) ? $mysqli->real_escape_string($_POST['telefone']) : $user_data['telefone'];

    if ($tipo_usuario == 'empreendedor') {
        $update_query = "UPDATE empreendedor SET nome = ?, email = ?, senha = ?, sexo = ?, data_de_nascimento = ?, cpf = ?, cep = ?, estado = ?, bairro = ?, rua = ?, complemento = ?, numero = ?, nome_perfil = ?, bibliografia = ?, cpf_cnpj = ?, telefone = ? WHERE id = ?";
        $stmt = $mysqli->prepare($update_query);
        $stmt->bind_param("ssssssssssssssssi", $nome, $email, $senha, $sexo, $data_nascimento, $cpf, $cep, $estado, $bairro, $rua, $complemento, $numero, $nome_perfil, $bibliografia, $cpf_cnpj, $telefone, $usuario_id);
    } else if ($tipo_usuario == 'investidor') {
        // Remova o campo `data_nascimento` da query e ajuste o bind_param
        $update_query = "UPDATE investidor SET nome = ?, email = ?, senha = ?, sexo = ?, cpf_cnpj = ?, cep = ?, estado = ?, bairro = ?, rua = ?, complemento = ?, numero = ?, nome_perfil = ?, bibliografia = ?, cpf_cnpj = ?, telefone = ? WHERE id = ?";
        $stmt = $mysqli->prepare($update_query);
        $stmt->bind_param("sssssssssssssssi", $nome, $email, $senha, $sexo, $cpf_cnpj, $cep, $estado, $bairro, $rua, $complemento, $numero, $nome_perfil, $bibliografia, $cpf_cnpj, $telefone, $usuario_id);
    }
    
    if ($stmt->execute()) {
        echo "<script>alert('Dados atualizados com sucesso!'); window.location.href = '$pagina_anterior';</script>";
    } else {
        echo "<script>alert('Erro ao atualizar dados: " . $mysqli->error . "'); window.location.href = 'pagina_anterior.php';</script>";
    }
...
<body>
    <header>
        ...
        <button style="all: unset; cursor: pointer;"><a href="principal-inv.php"> 
    ...
    <main>
        <div class="sidebar">
            <ul class="menu">
                ...
                <li class="menu-item" id="editar-perfil"><a href="editar-perfil-inv.php">Editar Perfil</a></li>
                <hr class="hr">
                <h1>Seus Dados Pessoais</h1>
                <li class="menu-item" id="dados-pessoais"><a href="editar-dados-pessoais-inv.php">Dados Pessoais</a></li>
            </ul>
        ...
        <div class="container">
            ...
            <div class="left">
                ...
                <input type="text" id="cpf-cpnj" placeholder="___.___.___-__" name="cpf" value="<?php echo htmlspecialchars($user_data['cpf_cnpj']); ?>">

                <label for="telefone">Telefone</label>
                <input type="text" id="telefone" placeholder="(xx) xx xxxxxxxxx" name="telefone" value="<?php echo htmlspecialchars($user_data['telefone']); ?>"><br>
            </div>
