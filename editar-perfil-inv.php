...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $cpf_cnpj = !empty($_POST['cpf_cnpj']) ? $mysqli->real_escape_string($_POST['cpf_cnpj']) : (isset($user_data['cpf_cnpj']) ? $user_data['cpf_cnpj'] : ''); // Alterado de cpf para cpf_cnpj
    ...
    $telefone = !empty($_POST['telefone']) ? $mysqli->real_escape_string($_POST['telefone']) : (isset($user_data['telefone']) ? $user_data['telefone'] : ''); // Adicionei telefone
    ...
    if (isset($_FILES['foto_perfil']) && $_FILES['foto_perfil']['error'] == UPLOAD_ERR_OK) {
        ...
        if (in_array($fileExtension, $allowedfileExtensions)) {
            ...
            $uploadFileDir = 'uploads/investidores/'; // Pasta onde as imagens serão armazenadas
            $dest_path = $uploadFileDir . uniqid() . '.' . $fileExtension;

            // Move o arquivo para a pasta de upload
            if(move_uploaded_file($fileTmpPath, $dest_path)) {
                $update_query = "UPDATE investidor SET nome = ?, email = ?, senha = ?, sexo = ?, cpf_cnpj = ?, cep = ?, estado = ?, bairro = ?, rua = ?, complemento = ?, numero = ?, nome_perfil = ?, bibliografia = ?, foto_perfil = ? WHERE id = ?";
                $stmt = $mysqli->prepare($update_query);
                $stmt->bind_param("ssssssssssssssi", $nome, $email, $senha, $sexo, $cpf_cnpj, $cep, $estado, $bairro, $rua, $complemento, $numero, $nome_perfil, $bibliografia, $dest_path, $usuario_id);
            } else {
        ...
        $update_query = "UPDATE investidor SET nome = ?, email = ?, senha = ?, sexo = ?, cpf_cnpj = ?, cep = ?, estado = ?, bairro = ?, rua = ?, complemento = ?, numero = ?, nome_perfil = ?, bibliografia = ? WHERE id = ?";
        $stmt = $mysqli->prepare($update_query);
        $stmt->bind_param("sssssssssssssi", $nome, $email, $senha, $sexo, $cpf_cnpj, $cep, $estado, $bairro, $rua, $complemento, $numero, $nome_perfil, $bibliografia, $usuario_id);
    }

    if ($stmt->execute()) {
        echo "<script>alert('Dados atualizados com sucesso!'); window.location.href = 'editar-perfil-inv.php';</script>"; // Substitua por sua página anterior
    } else {
    ...
    <main>
        <div class="sidebar">
            <ul class="menu">
                ...
                <li class="menu-item" id="dados-pessoais"><a href="editar-dados-pessoais-inv.php"><img src="Imagens-princ/outra-conta.png" alt="" width="20px">Dados Pessoais</a></li>
            </ul>
